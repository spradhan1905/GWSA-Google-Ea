...
export default function MetricSelector({ tabs = [], activeTab, onTabChange }) {
  return (
    <div className="shrink-0 flex border-b border-gwsa-border px-5 overflow-x-auto no-scrollbar">
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          <button
            ...
            className={`shrink-0 whitespace-nowrap flex items-center gap-1.5 px-3 py-2.5 text-xs font-medium transition-all duration-200 border-b-2 -mb-[1px] ${
              isActive
